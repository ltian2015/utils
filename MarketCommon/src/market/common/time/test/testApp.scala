...
object Test extends App{
  ...
  private def creatTcXML={
    val kunmingTc:TradeCenter=BaseTradeCenter("kunming","昆明电力交易中心","测试租户");
    val rootNode=
      <TradeCenters>
    {kunmingTc.toXML};
</TradeCenters>;
 ...
 scala.xml.XML.save(fileName, rootNode, "UTF-8", true, null);
  ...
  // creatIntervalConfig("kunming");
     val factory1=DefaultCalendarFactory.getInstance();
       val factory2=DefaultCalendarFactory.getInstance();
     val mc: MktCalendar=factory1.getCalendar("kunming")
